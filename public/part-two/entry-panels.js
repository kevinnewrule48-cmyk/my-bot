export function entryView(signal,live,experimental=false){
  ...
  const checks=signal.checks.filter(c=>c.name!=='Execution validation'&&(!experimental||!['Calibrated confidence','Payout / EV'].includes(c.name)));
  ...
  return {title:`${ready?'SETUP QUALIFIED':'WAIT'} · ${signal.type} ${signal.barrier}`,tone:ready?'qualified':'waiting',
    reason:ready?(experimental?'Experimental research signal ready. Server must check its live signal and actual payout before placing a demo order.':'Diagnostic checks passed. Execution approval is checked separately by the server.'):`Not a qualified entry. Waiting for: ${failed.map(c=>c.name).join(', ')}.`,
...
export function mountEntryPanels(){
  ...
  return (signal,market,experimental=false)=>{
    ...
    const view=entryView(signal,document.getElementById('status').textContent==='LIVE',experimental);
